...
public:
    ...
    int satisfiedCustomers(vector<int> &customers, vector<int> &grumpy, int start, int end, int initialSatisfied)
    {
        ...
        for (int i = start; i <= end; i++)
        {
            ...
            // means if in our window there was only 0 then on applying power
            // there will be no change and if there was 1 then it will make
            // changes to satisfied customers
            if (grumpy[i] == 1)
            {
    ...
    int maxSatisfied(vector<int> &customers, vector<int> &grumpy, int minutes)
    {
        ...
        // counting initial satisfied customer when our bookstore owner was not
        // grumpy
