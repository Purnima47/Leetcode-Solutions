class Solution {
    ...
    private int satisfiedCustomers(int[] customers, int[] grumpy, int start, int end, int initialSatisfied) {
        ...
        for (int i = start; i <= end; i++) {
            ...
            // means if in our window there was only 0 then on applying power there will be
            // no change
    ...
    public int maxSatisfied(int[] customers, int[] grumpy, int minutes) {
        ...
        while (endWindow < customers.length) {
            ...
            maxSatisfied = Math.max(maxSatisfied,
                    satisfiedCustomers(customers, grumpy, startWindow++, endWindow++, initialSatisfied));
        }
